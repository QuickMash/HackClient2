//This is the code you will want to copy
(() => {
...
.cheat:hover {
    ...
    GUI.style.fontFamily = 'Comic Sans';
    ...
    header.innerHTML = `<span style="color: gold;">Premium</span> Blooket Hacks <span style="font-size: 0.75rem">v 0.0.1</span>`;
    ...
    minimize.style.fontFamily = 'Sigmar';
    ...
    body.innerHTML = (`<span id="curPageEl">${getSite(true) ? `Current gamemode: ${getSite(true)}` : 'No game detected'}</span><br><span>(Press E to <span style="color: red">HIDE</span>)</span><br>`);
    ...
    footer.innerHTML = (`<span>A <span style="color: gold;">Premium</span> hack client by QuickMash Studios</span>`);
    ...
    let cheats = {
        global: {
            ...
            'ON/OFF Auto Answer': () => { autoAnswer = !autoAnswer },
            'ON/Off Highlight Answers': () => { highlightAnswers = !highlightAnswers },
            'Spam Open Boxes': () => {
                let box = prompt(`Which box do you want to open? (safari, aquatic, bot, space, breakfast, medieval, and wonderland)`);
                let boxes = {
                    safari: 25,
                    aquatic: 20,
                    bot: 20,
                ...
                if (!Object.keys(boxes).includes(box.toLowerCase())) return alert('Thats not on the list');
                let amount = prompt('Number to open');
                fetch("https://api.blooket.com/api/users", { credentials: "include" }).then(x => x.json()).then(x => {
                    if (x.tokens < boxes[box.toLowerCase()] * amount) amount = Math.floor(x.tokens / boxes[box.toLowerCase()]);
                    if (!amount) return alert('not enough tokens!!!!!!!!!!!');
        ...
        cafe: {
            'Infinite Food': () => {
                if (document.location.pathname != "/cafe") return alert("Leave the shop to use!");
                reactHandler().stateNode.state.foods.forEach(e => e.stock = 9999999999999999999999999);
                reactHandler().stateNode.forceUpdate();
            },
            'Max Levels': () => {
                if (document.location.pathname != "/cafe/shop") return alert("Open the shop to use!");
            ...
            if (choiceESP) {
                try {                   
                    ...
                    Object.entries(data.yes).forEach(x => {
                        ...
                        element.style = 'font-size: 24px; color: #ffffff; font-weight: bolder;';
